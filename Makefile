...
UID = $(shell id -u)
#UID = 197610
DOCKER_BE = order-app-code
NAMESERVER_IP = $(shell ip address | grep docker0)

ifeq ($(OS), Darwin)
	NAMESERVER_IP = host.dcoker.internal
...
start: ## Start the containers
	docker network create order-app-network || true
...
ssh-code: ## Bash into the be container
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} bash

code-style: ## Runs php-cs to fix code styling following Symfony rules
	U_ID=${UID} docker exec -i --user ${UID} ${DOCKER_BE} php-cs-fixer fix src --rules@Symfony
